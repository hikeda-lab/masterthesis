...
pdf("D4.pdf")
plot(c(rep(0,33), wavelet.filter("d4", f.seq), rep(0,33)), type="l",lwd=5,
       xlab="", ylab="", main="Wavelet filterD(4)")
dev.off()

pdf("D4red.pdf")
plot(c(rep(0,33), wavelet.filter("d4", f.seq), rep(0,33)), type="l",lwd=5,col=2,
       xlab="", ylab="", main="Wavelet filterD(4)")
dev.off()
