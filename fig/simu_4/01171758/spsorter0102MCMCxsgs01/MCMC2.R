...
spikesort.MCMC <- function()
  {
    ...
    delta.history <- matrix(nrow=MCMC.steps,ncol=num.neurons)
    ...
    for(now.step in 1:MCMC.steps)
      {
        ...
        for(label in 1:num.neurons){
          ...
          proposed.delta[label] =
            generate.random.walk.delta(label,spike.label,delta,lambda,delta.min,delta.max,now.steps)
          ...
          if( Accept > u.delta)
            {
              ...
              delta.history[now.step,label] <- delta[label]
        ...
        for(label in 1:num.neurons){
          ...
          proposed.lambda[label] =
            generate.random.walk.lambda(label,spike.label,delta,lambda,lambda.min,lambda.max,now.steps)
...
init <- function()
  {
   ...
   amp.time <- scan("./simu_data1224/SNR3max_amp.dat")[1:num.spikes] * 0.001
...
prob.amplitude <- function(lapsed.time,amp.value,delta,lambda)
  {
    ...
    ##
    mean = amp.value*(1-delta*exp(-lambda*lapsed.time))
    var = 1
    result = dnorm(1,mean,var)
    return(result)              
  }
prob.marginal.amplitude.delta <- function(lapsed.time,amp.value,delta,
                                           lambda.min,lambda.max)
  {
    ...
    delta.value <-  delta
    prob <- function(v)
      {    return(prob.amplitude(lapse,amp, delta.value, v)) }
...
prob.model <- function(label.number,firing.time,amp.value,spike.label,
                       ...
                       case=1)
  {
    ...
    prob.value =
      switch(case,
             prob.amplitude(lapsed.time,amp.value,delta,lambda),
             ##case 2:integrated on lambda
             prob.marginal.amplitude.delta(lapsed.time,amp.value,delta,lambda.min,lambda.max),
             ##case 3:integrated on delta
             prob.marginal.amplitude.lambda(lapsed.time,amp.value,lambda,delta.min,delta.max),
             ##case 4:integrated on delta,lambda
             prob.marginal.amplitude.spike.label(lapsed.time,amp.value,delta.min,delta.max,lambda.min,lambda.max),
             )
        ...
        for(j in 1:num.spikes)
          ...
          if(spike.label[j] == num.neurons)
            ...
            likelihood[i] = likelihood[i] *
              prob.model(i,firing.time,amp.time[i],spike.label,delta[spike.label[j]],
